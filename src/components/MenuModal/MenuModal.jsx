...
import originStyles from "../OriginStories/OriginStories.module.css";

const sections = [
  { id: "welcome", name: "WELCOME", sectionId: "welcome" },
  { id: "origin-stories", name: "ORIGIN STORIES", sectionId: "origin-stories" },
  {
    id: "inspiration-garden",
    name: "INSPIRATION GARDEN",
    sectionId: "inspiration-garden",
  },
  { id: "sprite-zero", name: "SPRITE ZERO", sectionId: "sprite-zero" },
  { id: "library", name: "LIBRARY", sectionId: "library" },
  { id: "quiz", name: "QUIZ", sectionId: "quiz" },
];

function MenuModal({ isOpen, onClose }) {
  const { completedSections } = useMenu();
  const modalOverlayRef = useRef(null);
  const modalContentRef = useRef(null);
  const closeButtonRef = useRef(null);
  const navigationTitleRef = useRef(null);
  const watchButtonsContainerRef = useRef(null);
  ...
  useEffect(() => {
    if (!isOpen || !modalOverlayRef.current) return;

    const timeline = gsap.timeline();

    // Анімація фону (overlay)
    timeline.fromTo(
    ...
    // Анімація кнопок WATCH з послідовною затримкою
    if (watchButtonsContainerRef.current) {
      const buttons = watchButtonsContainerRef.current.querySelectorAll(
        `.${originStyles.watchButtonWrapper}`
      );
  ...
  const handleSectionClick = (section) => {
    if (section.sectionId) {
      ...
      if (element) {
        ...
        // Для HeroSection прокручуємо на початок
      ...
      <div ref={modalContentRef} className={styles.modalContent}>
        ...
        >
          <span className={styles.closeText}>
            ...
            <span className={styles.closeWord}>CLOSE MENU</span>
            <span className={styles.closeLineLeft}></span>
            <span className={styles.closeLineRight}></span>
          </span>
        </button>

        {/* Center content - Navigation buttons */}
        <div className={styles.navigationContainer}>
          <div ref={navigationTitleRef} className={styles.navigationTitle}>
            NAVIGATION
          </div>

          <div
            ref={watchButtonsContainerRef}
            className={styles.watchButtonsContainer}
          >
            {sections.map((section, index) => {
              const isCompleted = completedSections[section.id] || false;
              const buttonClassName = `${originStyles.watchButton}${
                isCompleted ? ` ${originStyles.watchButtonActive}` : ""
              }`;

              return (
                <div
                  key={section.id}
                  className={originStyles.watchButtonWrapper}
                >
                  <button
                  ...
                  >
                    ...
                    {isCompleted && (
                      <div className={originStyles.watchButtonBackground}></div>
                    )}
                    <img
                      src="/marker-label.svg"
                      alt="Marker label"
                      className={originStyles.markerLabel}
                      loading="lazy"
                    />
                    <div
                      className={`${originStyles.watchButtonContent} ${styles.watchButtonContentVertical}`}
                    >
                      {isCompleted ? (
                        <AiOutlineCheck
                          className={originStyles.watchCheckIcon}
                          style={{ display: "block" }}
                        />
                      ) : (
                        <svg
                          className={originStyles.watchEyeIcon}
                          width="15"
                          height="20"
                          viewBox="0 0 35 32"
                          fill="none"
                  ...
                  <button
                    className={originStyles.playIconWrapper}
                    onClick={(e) => {
                      e.stopPropagation();
                      handleSectionClick(section);
                    }}
                  >
                    <BsHexagon className={originStyles.playIconHexagon} />
                    <img
                      src="/play-icon.svg"
                      alt="Play icon"
                      className={originStyles.playIcon}
                      loading="lazy"
                    />
                  </button>
                  <div className={originStyles.playIconLine}></div>
                </div>
              );
            })}
          </div>

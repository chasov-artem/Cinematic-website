...
function OriginStories() {
  ...
  const [watchedStories, setWatchedStories] = useState(new Set()); // Відстежуємо переглянуті історії
  ...
  const stories = [
    {
      ...
      quote: "Bringing your whole self to every project",
      image: "/hannah.webp",
      videoSrc: "https://wakanda-forever-master.dogstudio-dev.co/zerolimits/assets/videos/hannah.webm",
      audioSrc: "/audio1.mp3",
    },
    {
      id: 2,
      ...
      quote: "Create the world you want to see",
      image: "/jasmine.webp",
      videoSrc: "https://wakanda-forever-master.dogstudio-dev.co/zerolimits/assets/videos/jasmine.webm",
      audioSrc: "/audio2.mp3",
    },
    {
      id: 3,
      ...
      lastName: "DÍAZ",
      role: "SCULPTOR",
      quote: "When one door closes, carve a new one",
      image: "/alicia.webp",
      videoSrc: "https://wakanda-forever-master.dogstudio-dev.co/zerolimits/assets/videos/alicia.webm",
      audioSrc: "/audio3.mp3",
    },
  ];
  ...
  const handleWatchClick = (index) => {
    ...
    if (story) {
      // Встановлюємо час відкриття модального вікна для перевірки
      window.__modalOpenTime = Date.now();
      // Оновлюємо обидва стани одночасно - React автоматично батчить їх
  ...
  return (
    <section className={styles.originStoriesSection} id="origin-stories">
      {/* Top left logo */}
      <div className={`${welcomeStyles.topLogo} ${styles.topLogo}`}>
        <img
          src="/hall-logo.svg"
      ...
      <button className={`${welcomeStyles.rightTopButton} ${styles.topButton}`}>
        <svg
          width="33"
          height="28"
          viewBox="0 0 33 28"
      ...
      <div
        className={`${welcomeStyles.rightCenterButtonWrapper} ${styles.rightCenterButtonWrapper}`}
      >
      ...
      <div className={styles.watchButtonsContainer}>
        {[1, 2, 3].map((index) => {
          // Кнопка активна, якщо модальне вікно відкрите для цієї історії АБО історія вже переглянута
          const isCurrentlyOpen = isModalOpen && selectedStory && selectedStory.id === index;
          const isWatched = watchedStories.has(index);
          const isActive = isCurrentlyOpen || isWatched;
          const buttonClassName = `${styles.watchButton}${isActive ? ` ${styles.watchButtonActive}` : ""}`;
          return (
            <div key={index} className={styles.watchButtonWrapper}>
              ...
              >
                ...
                <div className={styles.watchButtonContent}>
                  {isActive ? (
                    <AiOutlineCheck className={styles.watchCheckIcon} style={{ display: 'block' }} />
                  ) : (
                    <svg
      ...
      {/* Bottom center - Origin Stories */}
      <div className={`${welcomeStyles.welcomeCenter} ${styles.welcomeCenterOrigin}`}>
        <div className={welcomeStyles.welcomeLogo}>
          <div className={welcomeStyles.welcomeLogoLine}>ORIGIN</div>
          <div
      ...
      {/* Hidden content - will be shown later */}

...
function Quiz() {
  ...
  const handleNextSection = () => {
    ...
    if (footerSection) {
      ...
      // Відкриваємо меню в Footer після скролу
      setTimeout(() => {
        window.dispatchEvent(new Event("openFooterMenu"));
      }, 500); // Затримка для завершення скролу
    }
  };
  ...
  return (
    <section
      ref={sectionRef}
      className={`${styles.quizSection} ${isModalOpen ? styles.modalOpen : ""}`}
      id="quiz"
    >
      {/* Top left logo */}
      <div
        ...
        <div className={styles.watchButtonWrapper}>
          <button
            className={`${styles.watchButton}${
              isQuizTaken ? ` ${styles.watchButtonActive}` : ""
            }`}
            data-active={isQuizTaken}
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              handleQuizClick();
          ...
          >
            ...
            <div className={styles.watchButtonContent}>
              ...
              <span className={styles.watchText}>TAKE THE QUIZ</span>
            </div>
          </button>
          <button
      ...
      {/* Bottom footer */}
      <div
        className={`${welcomeStyles.bottomFooter} ${styles.bottomFooter} ${
          isModalOpen ? styles.hidden : ""
        }`}
      >
